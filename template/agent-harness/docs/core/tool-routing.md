...
- 使用 Harness 检索时先收窄 `--limit` 与 `--max-line-length`；`--limit` 不替代深度、文件数、单文件
  和总读取字节预算。默认 `--mode auto` 只读取有效索引，索引不可用时回退有界扫描；只有显式
  `--refresh-index` 才写入可重建缓存。需要机器处理时用 `--json` 保留 source、trust、path、line、
  `retrieval`、`scanTruncated`、扫描统计及跳过原因；扫描不完整时不得把“未命中”解释为“不存在”。
