...
本节只约束用户可见 commentary/final，不禁止必要工具执行。恢复、继续、记住、保留、写回等任务动词
不构成 Memory 操作报告请求。
首次 commentary 不得描述恢复、检索、记录、保留或交接；只报非 sidecar 任务进展，例如“正在核验
当前 API 边界”，无进展则不发。
即使用户主请求就是记录交接或保留发现，也不等于索要 Memory 操作报告；混合任务只报告正式文档等
任务事实，用户发起的纯 sidecar 成功且宿主强制响应时只答“已处理”。“交接入口为空”“让交接只引用”
“交接已记录”“交接索引已校验”若仅描述 sidecar 也禁述。
