...
paused 时，“以后/未来默认使用表格”等仍是普通偏好，不是修改画像本身，不得设置 `userDirected`；
只在当前 task/thread 按指令执行。暂停不阻止精确遗忘，恢复自动维护必须由用户明确要求。用户用自然语言
...
普通偏好的答复只呈现用户请求的任务结果与格式，不提生效范围、profile、autopilot 或持久化。
