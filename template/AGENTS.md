...
- commentary/final只报非sidecar任务；Memory恢复/检索/记录/保留/交接/操作/状态/校验禁述，即使用户要求；纯sidecar成功仅答“已处理”；proposed/blocked或明示Memory审计例外；简中/原文/先结论

## 启动与发现（第3步后再commentary）

1. 新宿主 task/thread 首动作=静默读一次 `{{HARNESS_MEMORY_HOME}}/profile.md`；缺失继续；此前禁 pwd/Git/rg/find/ls/其他 Memory。
...
- 明示画像控制直跑 CLI --json；禁 docs/skill/help/预告；首条按格式仅结果/阻塞：
  纠正=reconcile-profile --payload-file <path> --json（paused 可，`userDirected:true`，单次不 resume）；遗忘=forget-profile --key <exact-key> --json；暂停/恢复=profile-autopilot pause|resume --json。
  遗忘按已读 profile 唯一匹配 exact-key 原样执行；0/多匹配阻塞，禁猜/“已替代/无需删除”跳过；敏感/冲突/越界提示。
- 写前验目标；禁 destructive Git清场；不泄露 secret/token/cookie/私钥。

## 按需路由

- 修改/诊断/评审/设计/发布→读`{{HARNESS_HOME}}/agent-harness/docs/README.md`→唯一命中playbook；工具/安全/Git/长任务/CLI→core/。
