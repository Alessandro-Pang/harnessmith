...
2. 读 `{{HARNESS_PERSONAL_HOME}}/AGENTS.md`，确认 cwd/Git 根/工作树/近端 `AGENTS.md`；跨项目列全局 Memory 元信息，读 `core.md`/命中正文。
3. 新宿主 task/thread 在绝对项目根执行 `test -d "<project-root>/.agent-docs"`；被 ignore，不得因 `rg`/Git 未命中判不存在。存在时首次读写前静默列元信息/读 `core.md`/查活跃 task 与命中正文；
   单文件不得跳过；缺失按标准判断，只读不初始化。
4. 读相关代码/配置/测试/manifest/lockfile/脚本；设计/计划不代表已实现。
5. 不递归加载整棵 `docs/`、`.agent-docs/`、历史会话或全部规则；由索引/检索命中最少正文。
6. 缺失若显著影响结果/权限/范围则询问，否则推进。
...
- 跨上下文/高风险/多阶段按需读 long-running task；简单/只读不建任务账本。
...
- 验收/scope/constraints 变化或含不可廉价恢复 source；项目 Memory 已有/任务应初始化时，去重后须先 capture-input --payload-file 再作任务改动，否则 proposal；无新信息不写，自动文本禁 shell 插值。
...
- 写前读 handoff/active task；handoff payload 每次必含 session/title/objective/completed/next/reason；未变 title/objective 从当前 handoff 原样带入；
  completed 累计，next=文件/命令/动作；未变 facts/decisions/open/verification/scope/sourceRefs 省略、不改写；原样重放除外；仅 resolved/superseded 才清理，模糊保留。
- workstream plan/backlog 已核验具体后续阶段；阶段完成并验证即“仍有后续”，本轮未授权也须最终答复前静默写 reason=phase handoff 并校验，不等下条用户消息。
- 同一会话完成多项任务/决策时，第二个独立任务验证后，最终答复前 reason=multi-task 累计写，后续原位更新；优先级 compaction > multi-task > phase。
...
- Harness 画像明示纠正/遗忘、autopilot 暂停/恢复：直接执行 CLI --json，不查产品文档/skill；禁预告，首条消息仅结果/阻塞，单句/格式优先。自动 reconcile 全程静默，proposed/blocked 简报；paused 偏好照做、不报画像/持久化；敏感/冲突/越界提示。
