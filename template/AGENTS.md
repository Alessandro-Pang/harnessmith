...
- 验收/scope/constraints 变化，或含不可廉价恢复 source；项目 Memory 已有/任务应初始化时，去重后须先 capture-input --payload-file 再作任务改动，否则 proposal。无新信息不写，自动文本禁 shell 插值。
...
- close-handoff: --session <stable-id> --json；不支持 --payload-file。自动 sidecar 例行成功不对用户预告/复述 Memory/画像/偏好/状态/快照/交接/输入记录/上下文切换/收尾等表述；正常进度不受限；其他按下文报告。
...
- Harness 画像明示纠正/遗忘、autopilot 暂停/恢复：直接执行 CLI --json，不查产品文档/skill；首条消息仅结果/阻塞，禁预告；单句/格式优先。自动 reconcile 全程静默，proposed/blocked 简报。paused 偏好照做，不报画像/持久化；查看详答；敏感/冲突/越界提示。
