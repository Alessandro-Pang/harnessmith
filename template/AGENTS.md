...
- 只报任务事实；读/写 Memory≠用户要报告；维护过程/结果改写亦禁述，Memory 审计/状态/清单请求除外；简中，术语/错误留英文；先结论证据。
...
- 确认 owner/调用链/边界/验收；多文件/高风险/跨仓用3–7步计划。
...
- 修改/诊断/评审/设计/发布：须先读恰好一个命中 playbook；工具/安全/Git/长任务/CLI→core/；项目 Memory→单独分段读 standards/project-agent-docs.md。
