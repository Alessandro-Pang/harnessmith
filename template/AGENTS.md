...
这里只保留每次适用、遗漏损失高且不能现场推断的边界；字段/流程按需路由，机械约束交给 CLI/schema。
...
- commit、push、merge、rebase、发布、生产迁移、远端写入、发消息、全局安装和不可逆删除都需要明确授权；宿主明确提供交互能力时，
  授权/澄清阻塞应主动调用并等待，结果只作用于当前动作，不写 Memory；无工具或 denied/cancelled/timeout 则 blocked 并给出 nextAction。

## 启动与发现（完成第 3 步前不发送 commentary）
...
4. 对修改、诊断、评审、设计、发布、工具、安全、Git、长任务或 CLI 请求，先读取
   ...
   references 仅按 owner 按需加载；omittedRequiredTopics 非空即停并报告，omittedTopics 延迟；无 intent 不猜，歧义停止。
   本地 Harness Memory/画像控制不是宿主产品设置；此范围不加载产品文档、skill、web，其他按路由。
5. 不递归读 `docs/`、`.agent-docs/`、archive、历史会话或全部规则；仅在缺失信息会改变权限、范围或结果时询问。

## 工作与交付

- 回复语言依次服从当前明确要求、持久证据和当前请求检测；保留必要 identifier、命令、路径和错误原文，先给结论。
