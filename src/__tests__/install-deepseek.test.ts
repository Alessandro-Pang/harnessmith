...
test('DeepSeek install, status, restore, and uninstall use the effective DSH home', () => {
  ...
  const upgraded = execute(root, ['install', '--agent', 'deepseek', '--no-init-global', '--json']);
...
test('DeepSeek aliases resolve to the deepseek adapter', () => {
  ...
  for (const agent of ['dsh', 'deepseek-harness', '6']) {
...
test('DeepSeek is preflighted before another target is changed', () => {
  const root = mkdtempSync(join(tmpdir(), 'harnessmith-deepseek-lifecycle-'));
  onTestFinished(() => rmSync(root, { recursive: true, force: true }));
  execute(root, ['--agent', 'codex,deepseek', '--no-init-global']);
  const codexRules = join(root, 'codex-home', 'AGENTS.md');
  const deepseekRules = join(root, 'dsh-home', 'AGENTS.md');
  writeFileSync(deepseekRules, `${readFileSync(deepseekRules, 'utf8')}\nuser edit\n`);

  const result = execute(root, ['uninstall', '--agent', 'codex,deepseek']);
  ...
  assert.ok(existsSync(join(root, 'dsh-home', '.harnessmith', 'install.json')));
});
