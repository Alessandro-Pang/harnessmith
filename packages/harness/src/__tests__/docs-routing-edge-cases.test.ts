...
test('a bare handoff request loads the long-running task owner', () => {
  const report = routeDocumentation(docsRoot, ['handoff']);
  assert.ok(
    report.topics.some(({ name }) => name === 'long-running-tasks'),
    'handoff must load the long-running task protocol',
  );
  assert.ok(
    report.requiredTopics.some(({ name }) => name === 'long-running-tasks'),
    'handoff must mark the long-running task protocol as required',
  );
  assert.ok(
    report.references.some(({ name }) => name === 'task-and-replay-contracts'),
    'handoff must expose its low-frequency field contract as a reference',
  );
});
...
test.each(['阶段交接', '关闭交接', '上下文预算信号'])(
  ...
  (query) => {
    const report = routeDocumentation(docsRoot, [query]);
    assert.ok(report.requiredTopics.some(({ name }) => name === 'long-running-tasks'));
...
test('debugging a long-running handoff chooses diagnose and loads only supporting topics', () => {
  ...
  assert.deepEqual(
    report.topics.map(({ name }) => name),
    ['long-running-tasks', 'operating-model', 'harness-cli-architecture', 'project-agent-docs'],
  );
});
