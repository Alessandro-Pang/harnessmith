...
外层 `packages/cli/src/` 必须知道 Codex Home、Cursor 项目目录、Claude 配置目录等宿主事实，也必须处理安装事务。内层
`packages/harness/` 则应该保持宿主中立，才能在不同 Agent 中复用同一套文档、Memory 和 Task 契约。
...
外层 `packages/cli/src/` 包含 CLI、Adapter、安全路径、操作锁、staging、备份、安装记录以及 restore/uninstall 事务。一次多宿主操作
...
工作区边界首先表达 owner 和依赖方向，不自动改变 npm 发布拓扑。Harness 不得依赖外层 CLI 或具体 Host Adapter；文档站点不得依赖 CLI 内部实现。只有完成独立构建、测试、版本与回滚验证后，工作区才可进一步拆分为独立发布包。
...
`packages/cli/src/adapters/adapter-registry.ts` 是宿主身份与 capability 的单一清单；`createAdapter()` 解析路径后挂上同一
...
`packages/cli/src/adapters/adapters.ts` 路径解析，并运行 `pnpm run eval:schema:generate`；preflight / `eval:schema:check`
