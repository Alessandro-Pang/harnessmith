...
Agent 只读取命中的正文，再回到代码、配置、测试和 schema 核对现场事实。历史文档可以继续保留用于追溯，但不会默认
进入每一次对话。这就是渐进披露：先发现，再读取，最后验证。

跨仓库任务会先从用户所有的 Personal overlay 读取 Repository Map，用它定位仓库职责、直接契约和证据路径，再到关系两侧的
代码、manifest、schema、测试或正式文档复核。Map 是跨仓决策索引，不是实时拓扑或替代项目事实的 CMDB。
