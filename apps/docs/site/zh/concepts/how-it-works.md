...
这一页按时间顺序讲完整条链路：装进机器时发生了什么、Agent 日常工作时它做什么、跨会话时它记什么、验证时它认什么。读完你应该能回答「我的规则到底存在哪、什么时候被读取、谁在执行」。文中的“五步”是面向使用者的阶段；架构页所说的“七步”是实现层的安全骨架（把路径复核、锁和提交拆开），两者描述的是同一事务的不同粒度。

## 安装时：先决定能不能写，再写

运行 `npx harnessmith install --agent codex` 之后，外层 CLI 按固定顺序工作。对用户来说，可以把它理解为下面 5 个阶段；每个阶段内部还会做多次路径和状态复核，前一阶段失败，后一阶段不会执行：
